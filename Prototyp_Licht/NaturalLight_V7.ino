...
// Anpassung an Wahrnehmung der Augen // 1024 Felder, umgekehrte Exponentialfunktion
int pwmtable[1024] =        
{
...
void setup() { 
  ...
  Serial.begin (115200); // Router als Server
  // WiFi.begin ( ssid, password );
  // WiFi.mode(WIFI_STA); // Schalte Accesspoint aus 
 ...
 WiFi.softAP ("Mina"); // Mikrocontroller = Server
  ...
  server.begin(); 
  webSocket.begin();
  webSocket.onEvent(webSocketEvent); // Interaktion Webseite -> Aufruf WebSocketEvent
}

void loop() {
// durchgehende Abfrage
  webSocket.loop(); 
  server.handleClient(); 
